...
### 新增/变更 - 指派与玩家名显示
- 优化“指派他人”界面：
  - 将玩家列表改为固定高度列表，支持鼠标滚轮滚动，防止玩家较多时按钮超出界面。
  - 在玩家列表上方新增搜索框，支持实时按玩家名过滤，方便快速定位目标玩家。
- 为团队任务增加 `assigneeName` 缓存字段：
  - 指派或领取任务时，服务器会根据指派人 UUID 解析出当前玩家名并写入任务数据，随团队任务一起持久化与同步。
  - GUI 列表与 HUD 在显示任务标签时，会优先从当前在线玩家列表获取最新玩家名；若玩家已离线，则回退使用任务中缓存的 `assigneeName`，保证已分配任务在“团队-已分配”视图和HUD中仍然显示指派人姓名。
