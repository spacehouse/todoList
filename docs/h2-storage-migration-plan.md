...
- M4-A 已完成项目侧栏计数 SQL 聚合，H2 后端走参数化查询，NBT 后端保留原内存路径。
- HUD 按范围、视图、星标项目、显示数量进行 SQL 查询，避免全量扫描。
- M4-B 已完成 HUD SQL 查询，H2 后端按项目来源、指派状态、完成状态和 limit 查询可绘制行，并返回总数保持隐藏计数和折叠摘要。
- GUI 搜索、优先级、完成状态、负责人、项目过滤逐步改为 DAO 查询。
- M4-C 已完成 GUI 筛选/搜索 SQL 查询，H2 后端按桶、项目、完成状态、优先级、指派状态和标题/描述/标签搜索返回任务 ID，再映射回当前内存对象以保持编辑语义。
- 大列表引入分页或 limit。
- M4-D 已补充 GUI 查询 limit/offset 和 count 能力；已完成分组折叠时仅查总数，展开或选中已完成任务时仍加载完整列表以保持滚动和编辑刷新语义。
- M4-E 已完成未完成分组虚拟滚动评估；未完成区因拖拽重排、选中定位和编辑刷新语义暂不分页，后续如需虚拟列表应单独设计。
